...
import { EmphasizedButton, EmphasizedButtonWithArrow, EmphasizedButtonWithBackArrow } from "./Buttons";
...
export default function FilmUpload() {
    ...
    return (
      <>
        ...
        <div class="flex items-center justify-center w-full p-2 px-4 bg-white">
          <input type="text" id="team_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 focus:outline-none"
        ...
        <div className="flex flex-row bg-gray-50 p-4">
          ...
          >
            ...
            <span className="block text-sm text-gray-600">
              Upload every clip from the game or the entire game as 1 video and enter the name of the team to scout
            </span>
          </div>
          <div className="ml-auto grid items-center flex-grow-0">
            <EmphasizedButtonWithArrow text={"Next"} link={"#"} onClick={handleNextClick} />
          </div>
        </div>
      </>
    );
  ...
  const ImageColumn = ({ images, checked }) => {
    return (
      <div className="flex flex-col justify-center space-y-4 bg-gray-50 border border-gray-300 rounded-lg block w-32 p-2.5 focus:outline-none">
        {images.map((image, index) => (
          <div key={index} className="w-16 h-16 rounded-lg overflow-hidden self-center">
  ...
  function TeamDescriptionTask() {
    return (
      <>
        ...
        <div className="relative grid gap-8 bg-white p-7 h-80 w-full">
            <RadioGroup value={selectedTeam} onChange={setSelectedTeam} className="flex justify-center space-x-8">
      ...
      <RadioGroup.Option value={2}>
        ...
        <div className="flex flex-row bg-gray-50 p-4">
          ...
          <div className="ml-auto grid items-center flex-grow-0 px-2">
            <EmphasizedButtonWithBackArrow text={"Back"} link={"#"} onClick={handleBackClick} />
          </div>
  ...
  let taskSet = [<FilmUploadTask />, <TeamDescriptionTask />]
